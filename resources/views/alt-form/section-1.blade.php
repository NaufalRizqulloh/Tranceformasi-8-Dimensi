      ...
      <div class="mt-4 md:mt-12 mx-auto w-[90%] md:w-[80%] lg:w-[60%] h-fit border-2 rounded-[30px] lg:rounded-[50px] bg-white dark:bg-slate-700 dark:border-transparent">
          ...
          <ul>
              @for ($i = 1; $i <= 4; $i++) <li class="flex h-auto">
                  <input type="checkbox" id="{{ $index . '-1-' . $i }}" class="hidden peer/{{ $index . '-1-' . $i }} single-checkbox{{ $index }}1 check{{ $index . $i }}" name="checkbox[p][{{ $index }}]" value="{{ $quest['value-p'][($i - 1)] }}" {{ (isset($answers['p'][$index]) && $answers['p'][$index] == $quest['value-p'][$i - 1]) ? 'checked' : ''}} />
                  ...
                  <input type="checkbox" id="{{ $index . '-2-' . $i }}" class="hidden peer/{{ $index . '-2-' . $i }} single-checkbox{{ $index }}2 check{{ $index . $i }}" name="checkbox[t][{{ $index }}]" value="{{ $quest['value-t'][($i - 1)] }}" {{ (isset($answers['t'][$index]) && $answers['t'][$index] == $quest['value-t'][$i - 1]) ? 'checked' : ''}} />
